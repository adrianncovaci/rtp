...
pub type CallerFuture<T> =
    Pin<Box<dyn Future<Output = Result<<T as Message>::Result>> + Send + 'static>>;

pub type CallerFn<T> = Box<dyn Fn(T) -> CallerFuture<T> + Send + 'static>;

pub type SenderFn<T> = Box<dyn Fn(T) -> Result<()> + 'static + Send>;
...
pub struct Caller<T: Message> {
    ...
    pub caller_fn: Mutex<CallerFn<T>>,
}
